...
<div class="relative"
    ...
    <div {{ $attributes->merge([
            'class' => 'flex flex-wrap items-center gap-x-2 gap-y-1 rounded-lg border bg-white px-2 py-2.5 transition sm:flex-nowrap '
        ...
        {{-- Pointer-only affordance; keyboard users reorder with the Move up / Move down buttons. --}}
        <span draggable="true"
              aria-hidden="true"
              ...
              x-on:dragend="reset()"
              class="flex h-7 w-5 shrink-0 cursor-grab items-center justify-center rounded text-gray-400 transition hover:text-gray-600 focus:outline-none focus-visible:ring-2 focus-visible:ring-indigo-500 active:cursor-grabbing">
        ...
        <button type="button"
                ...
                aria-pressed="{{ $field['selected'] ? 'true' : 'false' }}"
                class="flex min-w-0 flex-1 items-center gap-3 rounded text-left focus:outline-none focus-visible:ring-2 focus-visible:ring-indigo-500">
        ...
        <div class="ml-auto flex shrink-0 items-center gap-0.5">
            <x-builder::icon-button
                ...
                wire:target="moveFieldUp('{{ $fieldId }}')"
            ...
            <x-builder::icon-button
                ...
                wire:target="moveFieldDown('{{ $fieldId }}')"
                wire:loading.attr="disabled">
                <svg class="h-4 w-4" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" aria-hidden="true">
            ...
            <x-builder::icon-button
                ...
                wire:target="duplicateField('{{ $fieldId }}')"
                wire:loading.attr="disabled">
                <svg class="h-4 w-4" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" aria-hidden="true">
            ...
            <x-builder::icon-button
                ...
                wire:target="removeField('{{ $fieldId }}')"
                wire:loading.attr="disabled">
                <svg class="h-4 w-4" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" aria-hidden="true">
